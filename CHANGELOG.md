...
- `/unfollow <player>` Discord slash command to stop following
  a player (previously only available via the button on
  `/profile`).
- `/confess <message>` Discord slash command that posts an
  anonymous confession to a configured `channels.confessions`
  channel. Rate-limited to one confession per player per
  configurable cooldown (`confessions.cooldown`, default 300s).
  Each confessor gets a stable monotonic handle. `&` colour
  codes are converted to Discord markdown.
- Silent update check every 5 hours (was 6). When a new release
  is detected, a single quiet embed is posted to
  `misc.update-channel` (no pings, no looping). The
  `misc.update-silent` flag suppresses both the in-game admin
  banner and the Discord notice for operators who want to keep
  console as the only signal.
- Achievement **rarity display**: the advancement announcement
  ...
  than a configurable threshold (default 25%) of active players
  have ever unlocked it.
...
- `FollowModule` keeps an in-memory cache of player→follower
  ...
  `User.openPrivateChannel()` API (fully non-blocking). The
  cache is populated lazily on the first join and refreshed on
  add/remove.
...
- `PlayerProfileBuilder` util — single source of truth for the
  profile-card embed. Resolves the linked Discord username
  (not just the raw ID) for a richer profile card.
- Configurable confession appearance: `confessions.color`
  (default `#9B59B6`) and `confessions.cooldown` (seconds).
- Configurable advancement rarity: `events.advancement.show-rarity`
  (default true) and `events.advancement.rarity-threshold`
  (default 0.25).
- `follow.enabled` config toggle to disable all follow features.
- MySQL `isFollowing` uses an efficient `SELECT COUNT(*) > 0`
  query instead of fetching the full follower set.

### Changed
- `YamlStorage` and `MySQLStorage` grew three new files / three
  new tables each, plus 12 new methods on the
  `StorageManager` interface.
- `AdvancementListener` now persists the unlock to storage,
  reads the rarity stats in an async hop, and guards against
  duplicate events (e.g. from a `/reload`). The generic
  "Unlocked by" field is suppressed for duplicates.
- `UpdateChecker` interval is now 5 hours (was 6). The
  `postSilentDiscordNotice(...)` helper only fires once per
  detected release thanks to an `AtomicBoolean` guard, and now
  respects `misc.update-silent`.
- `JoinQuitListener` writes `last_seen` and increments the
  ...
  abnormally.
- First-join detection uses storage (`getFirstJoin(uuid) == 0`)
  instead of `player.hasPlayedBefore()`, which is unreliable
  after a `/reload`.
...
### Fixed
- `FollowModule.onPlayerJoin` no longer blocks the scheduler
  thread — `retrieveUserById().complete()` replaced with
  `.queue()`.
- `MySQLStorage.isFollowing` now uses a lightweight
  `SELECT COUNT(*)` query instead of fetching all followers.
- `/seen` shows "This player has never joined the server"
  instead of generic "No activity recorded yet."
- Follow DM notification now includes a Discord timestamp of
  when the player joined.
- Profile card Discord section shows the actual username
  (e.g. `User#1234`) instead of just the raw numeric ID.
- Confession handles use a monotonic `AtomicInteger` counter
  instead of the collision-prone `hash % 10000` formula.
- `/panel` no longer crashes with an unhandled exception when
  used in a thread or forum channel; it replies with a helpful
  error message instead.
- `UpdateChecker` Discord announcement no longer gets permanently
  lost if the bot is disconnected at detection time; the flag
  is reset on failure so the next check retries.
- Confession cooldown is now applied only after the message is
  successfully sent; a failed confession no longer locks the
  user out of the cooldown period.
...
- UpdateChecker re-runs every 5 hours so long-running servers pick
